...
export class ContextMenu extends Menu {
  ...
  init() {
    ...
    document.body.addEventListener(
      'contextmenu',
      (event) => this.open(event.offsetY, event.offsetX) && event.preventDefault(),
    );
    ...
    this.el.addEventListener(
      ...
      (event) =>
        ...
        this.#modules
          .find(
            (mod) =>
              mod.type === event.target.dataset.type &&
              (this.#log.setLog = `Switch '${mod.text}' on!`),
          )
          ?.trigger(),
    );
